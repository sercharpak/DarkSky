...
# Constants
#-------------------------------------------
USAGE = "python volume_plots_awk.py input_folder thresh_VDH scale_factor n_grid output_folder"
#-------------------------------------------
#Functions
#-------------------------------------------
...
for thresh_seeds_FA in array_seeds_FA:
    int_thresh_FA = int(10*thresh_seeds_FA)
    str_suffix = "0"+str(int_thresh_FA)+"09"
    inputfile = input_folder+"/"+str_suffix+"/v_groups_vol_"+str_suffix+".dat"
    print ("Processing the file: " + inputfile)
    filename = path_leaf(inputfile)
    thresh_search_FA=0.9
    thresh_search_Trace = 0.0
    thresh_seeds_Trace = thresh_VDH
    group_data = np.loadtxt(inputfile)
    n_groups = file_len(inputfile)
    print ("There are", n_groups,"groups")
    #---------------------------Volume distribution----------------
    if(n_groups==1):
        volumes = group_data[1]
    else:
        volumes = group_data[:,1]
